...
const selectOptions: Array<Option> = [
  { name: "default", text: "Sort" },
  { name: "createdAt", text: "Created" },
  { name: "title", text: "Title" },
  { name: "updatedAt", text: "Modified" },
  { name: "recipients", text: "Sent" },
  { name: "responses", text: "Recieved" },
];
...
const DashboardPage = () => {
  ...
  const { data, isLoading, error, refetch, isFetching } = useGetSurveysQuery(
    { page, sort },
    {
      ...
      <Head>
            ...
            <div className="flex flex-col items-center p-5 space-y-5 md:flex-row md:justify-between md:space-y-0">
              <div className="flex flex-col items-center space-y-2 md:flex-row md:space-x-3 md:space-y-0">
                <CustomImage
                  priority
                  src={user?.imageUrl}
                  fallbackText={getNameInital(user?.name!)}
                  alt="profile image"
                />
              ...
              <div className="flex items-center space-x-4">
                <Link
                  className="bg-secondary inline-block text-white px-2 py-1 outline-none rounded-md duration-150 transition-colors hover:bg-green-700"
                  href="/dashboard/fund"
                >
                  Add Credits
                </Link>
                <Link
                  className="inline-block text-secondary px-2 py-1 outline-none rounded-md duration-150 transition-colors border border-secondary hover:bg-green-700 hover:text-white hover:border-white"
        ...
        <section className="mt-16 flex-1">
          <Container className="h-full flex flex-col flex-1">
            ...
            <div className="mt-8 md:mt-10 flex flex-col flex-1 space-y-4">
              <div className="flex-1 space-y-2">
                ...
                {data && data.surveys.length === 0 && !isFetching && (
                  <div className="text-center">No Data was found</div>
                )}
                {data &&
              ...
              {data && <Pagination {...data.paginate} className="self-end shrink-0" />}
            </div>
          </Container>
        </section>
    </div>
